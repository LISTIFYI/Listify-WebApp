...
const VideoDetails = ({ handleCloseDetails, post, }: VideoDetailsProps) => {
    ...
    const getDetailsById = async () => {
        setLoading(true)
        try {
            const response = await axios.get(`/public/listings-v2/${post?.listing?.id}`)
    ...
    return (
        <>
            {
                loading ? <div className='transition-all h-full flex justify-center items-center flex-col gap-2'>
                    <h1 className='font-medium text-[16px]'>Property Details</h1>
                    <h1 className='animate-spin duration-300'><LoaderCircle /></h1>
                </div> :
                    <div className="h-full flex flex-col  overflow-y-auto transition-all">
                        ...
                        <div className="bg-black text-white flex flex-col">
                            ...
                            <div className="p-4 space-y-4 pb-28">
                                {/* Price + Location */}
                                <div className=''>
                                    <h1 className="text-[18px] font-semibold">Price ₹{post?.listing?.pricing?.amount?.toLocaleString() || "N/A"}</h1>
                                            ...
                                            <CarouselContent className="flex gap-4">
                                                {Array.from({ length: 5 }).map((_, i) => (
                                                    <CarouselItem key={i} className="basis-auto">
                                                        <div className="bg-gray-900 p-3 rounded-xl min-w-[340px] inline-flex justify-center">
                                                            <div className='flex flex-row gap-4'>
                                                                <div className='border h-full w-[120px] rounded-md'> </div>
                                                                <div>
                                                                    ...
                                                                    <p className="text-white text-[14px]">₹{post?.listing?.pricing?.amount.toLocaleString() || "N/A"} Onwards - ₹{(post?.listing?.pricing?.amount / 100).toLocaleString()} EMI</p>
